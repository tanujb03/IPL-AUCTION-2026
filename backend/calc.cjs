...
const allPlayers = parseCSV(path.join(__dirname, 'resources/sequence_4.csv'));
const allRiddles = parseRiddles(path.join(__dirname, 'resources/sequence_4_riddles.txt'));
...
const brandBonus = Number(brandScore / maxBrandScore * 5); // From scoringService.js: BrandBonus = NormalizedBrand * 5. NormalizedBrand = team.brand_score / max(all brand_scores)
// Wait: calcBrandBonus = function(teamBrandScore) { return Number(teamBrandScore); } in scoringService - NO, wait.
// Scoring service has a small bug or I read it wrong? 
// 166: function calcBrandBonus(teamBrandScore) { return Number(teamBrandScore); }
// BUT calculateLeaderboard passing maxBrandScore:
// 282: const brandBonus = calcBrandBonus(team.brand_score, maxBrandScore);
// Since calcBrandBonus ignores maxBrandScore, it just returns teamBrandScore directly!!
// Let's use what the service actually does. teamBrandScore = 40.00
const actualBrandBonus = Number(brandScore);

const finalScore = bestScore + balanceBonus + efficiencyBonus + actualBrandBonus;
...
best11.sort((a,b)=>b.rating - a.rating).forEach(p => {
    ...
    console.log(`- ${p.rank} ${p.name} [${p.category}/OVS:${p.nationality==='OVERSEAS'}/Rating:${p.rating}] ${lbl}`);
});

console.log(`\nBase Score + Cap/VC: ${bestScore.toFixed(2)}` );
console.log(`Balance Bonus: ${balanceBonus}`);
console.log(`Efficiency Bonus: ${efficiencyBonus}`);
console.log(`Brand Bonus: ${actualBrandBonus.toFixed(2)}`);
